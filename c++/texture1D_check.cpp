...
while(x<stride.size())
{
	...
	{	
		if(loop==loop_next && warp/4==warp_next/4)
		{
			...
			{
				if(L1_seg[L1_find][index_next*size_of[array_next]%128/32]==1)
				{
				L1_hit += 1;
				...
				L1.erase(L1_find+L1.begin());
				L1.push_back(index_next*size_of[array_next]/L1_cache_line);
                                L1_seg[L1_find][index_next*size_of[array_next]%128/32]=1;
                                L1_seg.push_back(L1_seg[L1_find]);
				L1_seg.erase(L1_find+L1_seg.begin());
				}
			...
			{continue;}
				...
				if (rw_next == 0)
				 {
					...
					{
						L1.erase(L1.begin());
						L1_seg.erase(L1_seg.begin());
					}
					
					L1.push_back(index_next*size_of[array_next]/L1_cache_line);
					std::vector<long> temp;
					temp.push_back(0);temp.push_back(0);temp.push_back(0);temp.push_back(0);
					temp[index_next*size_of[array_next]%128/32]=1;
					L1_seg.push_back(temp);
				}
				...
				{	
					if(L2_seg[L2_find][index_next*size_of[array_next]%128/32]==1)
					L2_hit += 1;
					L2_seg[L2_find][index_next*size_of[array_next]%128/32]=1;
					L2.erase(L2_find+L2.begin());
					L2.push_back(index_next*size_of[array_next]/L2_cache_line);
					L2_seg.push_back(L2_seg[L2_find]);
					L2_seg.erase(L2_find+L2_seg.begin());
				}
				...
				else if(rw_next==0&&L2_find>=L2.size()&&collect_find>=collect_cache_line.size())
				{
					...
					{
						L2.erase(L2.begin());
						L2_seg.erase(L2_seg.begin());
					}
					L2.push_back(index_next*size_of[array_next]/L2_cache_line);
					std::vector<long> temp;
					temp.push_back(0);temp.push_back(0);temp.push_back(0);temp.push_back(0);
					temp[index_next*size_of[array_next]%128/32]=1;
					L2_seg.push_back(temp);
				}
			}
