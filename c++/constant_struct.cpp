...
while(x<stride.size())
{
	...
	{	
		...
		{
			index = index_next;
			collect_find = find(collect_cache_line.begin(),collect_cache_line.end(),index_next)-collect_cache_line.begin();
			...
			collect_cache_line.push_back(index_next);
		}
		else{	
		...
		if(rw_next==1&&rw==1)
		{
			...
			{
			output<< "constant "<<array<<" "<<exp<<" 9999 9999 9999\n";
		...
		output<< "constant "<<array<<" "<<exp<<" "<<L1_hit<<" "<<L2_hit<<" "<<total_cache_line-L1_hit-L2_hit<<"\n";
