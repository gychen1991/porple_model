...
int whole_line;
std::vector<int> set;
cache_level(int one=0,int two=0,int three=0,int four=0,int five=0);
};
struct is_valid
{
...
cache_level::cache_level(int one,int two,int three, int four,int five)
{
        whole_line=one;
        set.push_back(two);
        set.push_back(three);
		...
		if(loop==loop_next && warp/4==warp_next/4)
		{
			...
			{continue;}
				...
				if (rw_next == 0)
				 {
					...
					cache_level temp(index_next*size_of[array_next]/L1_cache_line,0,0,0,0);
				...
				else if(rw_next==0&&L2_find>=L2.size()&&collect_find>=collect_cache_line.size())
				{
					...
					cache_level temp(index_next*size_of[array_next]/L2_cache_line,0,0,0,0);;
					temp.set[index_next*size_of[array_next]%128/32]=1;
					L2.push_back(temp);
				}
			}
