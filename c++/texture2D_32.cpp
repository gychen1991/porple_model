...
struct is_valid
{
        is_valid(int a_wanted_x,int a_wanted_y):wanted_x(a_wanted_x),wanted_y(a_wanted_y){}
        int wanted_x,wanted_y;
        bool operator()(cache_level tofind)
        {
                return (tofind.whole_line_x==wanted_x&&tofind.whole_line_y==wanted_y);
        }
};
...
std::vector<cache_level> collect_cache_line;
//std::vector<long> L1;
//std::vector<long> L2;
...
while(x<stride.size())
{
	...
	{	
		...
		{
			index = index_next;
			collect_find = find_if(collect_cache_line.begin(),collect_cache_line.end(),is_valid(index_next%block_X[array_next]*size_of[array_next]/16,index_next/block_X[array_next]/2))-collect_cache_line.begin();
			index_x=index_next%block_X[array_next]*size_of[array_next]/32;
			index_y=index_next/block_X[array_next]/4;		
			L1_find = find_if(L1.begin(),L1.end(),is_valid(index_x,index_y))-L1.begin();
			if(rw_next ==0 && L1_find < L1.size() \
			&&collect_find>=collect_cache_line.size()  \
			)
			{

				if(L1[L1_find].set[long(index_next/block_X[array_next]%4/2)*2+index_next*size_of[array_next]%block_X[array_next]%32/16]==1)
				{
				L1_hit += 1;
				...
				L1[L1_find].set[index_next/block_X[array_next]%4/2*2+index_next*size_of[array_next]%block_X[array_next]%32/16]=1;
                                L1.push_back(L1[L1_find]);
				L1.erase(L1_find+L1.begin());
				L2_find = find_if(L2.begin(),L2.end(),is_valid(index_next%block_X[array_next]*size_of[array_next]/32,index_next/block_X[array_next]/4))-L2.begin();
                                if(rw_next==0&&L2_find<L2.size() \
                                   && collect_find >= collect_cache_line.size() \
                                  )
                                {
                                        if(L2[L2_find].set[index_next/block_X[array_next]%4/2*2+index_next*size_of[array_next]%block_X[array_next]%32/16]==1)
                                        {L2_hit += 1; }
                                        L2[L2_find].set[index_next/block_X[array_next]%4/2*2+index_next*size_of[array_next]%block_X[array_next]%32/16]=1;
                                ...
                                else if(rw_next==0&&L2_find>=L2.size()&&collect_find>=collect_cache_line.size()&&L2_lines>0)
                                {
                                        ...
                                        cache_level temp={};
                                        temp.whole_line_x=index_next%block_X[array_next]*size_of[array_next]/32;
                                        temp.whole_line_y=index_next/block_X[array_next]/4;
                                        temp.set[index_next/block_X[array_next]%4/2*2+index_next*size_of[array_next]%block_X[array_next]%32/16]=1;
                                        L2.push_back(temp);
				}
				}
			...
			{continue;}
				...
				{	
					if(L2[L2_find].set[index_next/block_X[array_next]%4/2*2+index_next*size_of[array_next]%block_X[array_next]%32/16]==1)
					{L2_hit += 1;if(array_next==1) cout<<index_next<<endl; }
					L2[L2_find].set[index_next/block_X[array_next]%4/2*2+index_next*size_of[array_next]%block_X[array_next]%32/16]=1;
					L2.push_back(L2[L2_find]);
					L2.erase(L2_find+L2.begin());
				}
			...
			{
			cache_level temp={};
                        temp.whole_line_x=index_next%block_X[array_next]*size_of[array_next]/16;
                        temp.whole_line_y=index_next/block_X[array_next]/2;
			
			collect_cache_line.push_back(temp);
			}
		}
		else{	
			...
			collect_find = find_if(collect_cache_line.begin(),collect_cache_line.end(),is_valid(index_next*block_X[array_next]*size_of[array_next]/16,index_next/block_X[array_next]/2))-collect_cache_line.begin();
                        L1_find = find_if(L1.begin(),L1.end(),is_valid(index_next%block_X[array_next]*size_of[array_next]/32,index_next/block_X[array_next]/4))-L1.begin();
                        if(rw_next ==0 && L1_find < L1.size() \
			&&collect_find>=collect_cache_line.size()  \
			)
			{
				if(L1[L1_find].set[index_next/block_X[array_next]%4/2*2+index_next*size_of[array_next]%block_X[array_next]%32/16]==1)
				{
				L1_hit += 1;
				//std::vector<int> temp=(0,0,0,0);
				//temp[index_next]=1;
		...
		if(rw_next==1&&rw==1)
		{
			...
			{
			output<< "texture2D "<<array<<" "<<exp<<" 9999 9999 9999\n";
		...
		output<<"texture2D "<<array<<" "<<exp<<" "<<L1_hit<<" "<<L2_hit<<" "<<total_cache_line-L1_hit-L2_hit<<"\n";
	...
	/*	for(int i =0 ;i <L1_seg.size();i++)
		...
		collect_cache_line.clear();
		cache_level temp_collect={};
                temp_collect.whole_line_x=index_next%block_X[array_next]*size_of[array_next]/16;
                temp_collect.whole_line_y=index_next/block_X[array_next]/2;
                        
                collect_cache_line.push_back(temp_collect);
