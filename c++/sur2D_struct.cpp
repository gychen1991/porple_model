...
int tile_x=32;
int tile_y=4;
if(L1_cache_line!=128)  //if not 128 then must be 32 or 0
{	tile_x=tile_x/2;
	tile_y=tile_y/2;
}
ofstream output;
output.open("output_DP2.txt",std::ofstream::app);
while(x<stride.size())
{
	...
	{	
		...
		{
			index = index_next;
			collect_find = find_if(collect_cache_line.begin(),collect_cache_line.end(),is_valid(index_next%block_X[array_next]*size_of[array_next]/tile_x,index_next/block_X[array_next]/tile_y))-collect_cache_line.begin();
			index_x=index_next%block_X[array_next]*size_of[array_next]/tile_x;
			index_y=index_next/block_X[array_next]/tile_y;		
				...
				if (rw_next == 0 && L1_lines>0)
				 {
					...
					cache_level temp={};
					temp.whole_line_x=index_next%block_X[array_next]*size_of[array_next]/tile_x;
					temp.whole_line_y=index_next/block_X[array_next]/tile_y;
				//	temp.set[index_next/block_X[array_next]%4/2*2+index_next*size_of[array_next]%block_X[array_next]%32/16]=1;
					L1.push_back(temp);
				}
				L2_find = find_if(L2.begin(),L2.end(),is_valid(index_next%block_X[array_next]*size_of[array_next]/tile_x,index_next/block_X[array_next]/tile_y))-L2.begin();
				if(L2_find<L2.size() \
				   && collect_find >= collect_cache_line.size() \
				  )
				{	
			...
			collect_find = find_if(collect_cache_line.begin(),collect_cache_line.end(),is_valid(index_next*block_X[array_next]*size_of[array_next]/tile_x,index_next/block_X[array_next]/tile_y))-collect_cache_line.begin();
                        L1_find = find_if(L1.begin(),L1.end(),is_valid(index_next%block_X[array_next]*size_of[array_next]/tile_x,index_next/block_X[array_next]/tile_y))-L1.begin();
                        if(rw_next ==0 && L1_find < L1.size() \
			&&collect_find>=collect_cache_line.size()  \
			)
			{
		...
		/*if(rw_next==1&&rw==1)
		{
			...
			{
			output<<"surface "<<array<<" "<<exp<<" "<<"99999999 99999999 99999999\n";
		...
		output<<"surface "<<array<<" "<<exp<<" "<<L1_hit<<" "<<L2_hit<<" "<<total_cache_line-L1_hit-L2_hit<<"\n";
	...
	/*	for(int i =0 ;i <L1_seg.size();i++)
		...
		cache_level temp_collect={};
                temp_collect.whole_line_x=index_next%block_X[array_next]*size_of[array_next]/tile_x;
                temp_collect.whole_line_y=index_next/block_X[array_next]/tile_y;
