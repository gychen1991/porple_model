...
struct Band* bb(long long i, long long j, long long band1,long long band2,vector<long long> &D2D1,vector<long long> &l_s){
	vector<long long> l2;
	struct Band* min=(struct Band *)malloc(sizeof(struct Band));
	if(j==0&&D2D1[i]==0)
		band1+=global[i][0]*glob_lat[2]+global[i][1]*glob_lat[1]*4+global[i][2]*glob_lat[0]*4;
	...
	for(long long k=0;k<2;k++)
	{
		...
		if (minimal>max(lastmin->GL,lastmin->TX))
			{
...
for(long long i =0;i<instr.size();i++)
{
	...
	{
		if(std::stoi(worklist[unprev][3])*con_lat[2]+std::stoi(worklist[unprev][4])*con_lat[1]+std::stoi(worklist[unprev][5])*con_lat[0]<= \
...
for(long long i=0;i<instr.size();i++)
{
	if(instr[i]!=0) instr_after.push_back(instr[i]);
}
...
for(long long i=0;i<instr_after.size();i++){
	...
	sum1D += texture1D[location][0]+texture1D[location][1]+texture1D[location][2];
	sum2D += texture2D[location][0]+texture2D[location][1]+texture2D[location][2];
	location++;
	}
	if(sum1D<sum2D)
	{
